...
\033[1;31m╚══════╝ ╚══╝╚══╝ ╚══════╝╚══════╝   ╚═╝
{trang}═══════════════════════════════════════════════════════════
{thanh} {trang}Developed By: {xanh}Sweet Tool  
{thanh} {trang}Admin: {xanh}Trung Sơn 
{trang}═══════════════════════════════════════════════════════════
{thanh} {trang}Youtube: {xanh}No
{thanh} {trang}Box Zalo: {xanh}https://zalo.me/g/uxapkc369  
{trang}═══════════════════════════════════════════════════════════
{thanh} {trang}Name Tool: {xanh} Trao Đổi Sub - Facebook
{trang}═══════════════════════════════════════════════════════════""")
banner()
def countdown_(delay):
    for remaining in range(delay, 0, -1):
        for _ in range(5):  # Tăng tốc hiệu ứng "x" bằng cách hiển thị 5 lần mỗi giây
            ...
            print(f" \r{do}[{vang}THTOOL{do}] {do}[{trang}Countdown{do}] {do}[{luc}{effect}{do}] [{xnhac}{remaining}{do}]                         \r", end='')
            sleep(0.2)  # Giảm thời gian chờ để hiệu ứng nhanh hơn
    print("\r\r\033[1;95m    ⚡THTOOL⚡\033[1;37m                         \r", end='')
    print(" "*65, end="\r")
def loadjob(delay):
    for remaining in range(delay, 0, -1):
        for _ in range(5):  # Tăng tốc hiệu ứng "x" bằng cách hiển thị 5 lần mỗi giây
            effect = "".join(random.choice(["𝐗", " "]) for _ in range(5))  # Chuỗi "x" ngẫu nhiên
            print(f" \r{do}[{vang}THTOOL{do}] {do}[{trang}Load_Job{do}] {do}[{luc}{effect}{do}] [{xnhac}{remaining}{do}]                         \r", end='')
            sleep(0.2)  # Giảm thời gian chờ để hiệu ứng nhanh hơn
    print("\r\r\033[1;95m    ⚡THTOOL⚡\033[1;37m                         \r", end='')
    print(" "*65, end="\r")
def DelayTime(delay):
    for remaining in range(delay, 0, -1):
        for _ in range(5):  # Tăng tốc hiệu ứng "x" bằng cách hiển thị 5 lần mỗi giây
            effect = "".join(random.choice([".", " "]) for _ in range(5))  # Chuỗi "." ngẫu nhiên
            print(f" \r{do}[{vang}Sweet{do}] {do}[{trang}Delay{do}] {do}[{luc}{effect}{do}] [{xnhac}{remaining}{do}]                         \r", end='')
            sleep(0.2)  # Giảm thời gian chờ để hiệu ứng nhanh hơn
    print("\r\r\033[1;95m    ⚡Sweet⚡\033[1;37m                         \r", end='')
    print(" "*65, end="\r")
def gio_vn():
...
def show_task_menu():
    print(f"{atuan}{luc}Chọn Loại Nhiệm Vụ {do}({luc}Ví Dụ: {vang}1,2,5{do})")
    [print(f"{atuan}{luc}Nhập {do}[{vang}{k}{do}] {luc}{v[1]}") for k, v in TASKS.items()]
    print(f"{atuan}{luc}Nhập {do}[{vang}X{do}] {luc}All Nhiệm Vụ")
    chon = input(f"{atuan}{luc}Nhập Số: {vang}").split(",")
...
def main():
    ...
    if not old_token:
        token = input(f"{atuan} {luc}Nhập Access Token TDS: {vang}")
        tds = TDS_API(token)
        profile = tds.profile()
        ...
        print(f"{atuan}{luc}Nhập {do}[{vang}1{do}] {luc}Giữ Tài Khoản {vang}{user}")
        print(f"{atuan}{luc}Nhập {do}[{vang}2{do}] {luc}Nhập Access Token TDS Mới")
        if input(f"{atuan}{luc}Nhập Lựa Chọn: {vang}") == "2":
            token = input(f"{atuan}{luc}Nhập Access Token TDS: {vang}")
            tds = TDS_API(token)
            profile = tds.profile()
            if 'error' in profile: print(atuan+do, profile['error']); return
    ...
    banner()
    print(f"{atuan}{luc}Tài Khoản: {vang}{user}")
    print(f"{atuan}{luc}Xu Hiện tại: {vang}{xu}")
    print(f"{atuan}{luc}Xu Die: {vang}{xudie}")
    print(f"{trang}═══════════════════════════════════════════════════════════")
    ...
    while True:
        stt += 1
        cookie_fb = input(f'{atuan}{luc}Nhập Cookie Facebook Thứ {trang}{stt}: {vang}')
        if cookie_fb == '': break
    ...
    if not job_types: print(f"{atuan}{do}Chọn Sai Rồi Bro ((="); exit()

    print(f"{trang}═══════════════════════════════════════════════════════════")
    delay = int(input(f"{atuan}{luc}Nhập Delay: {vang}"))
    nvblock = int(input(f"{atuan}{luc}Sau Bao Nhiêu Nhiệm Vụ Thì Nghỉ: {vang}"))
    delaybl = int(input(f"{atuan}{luc}Thời Gian Nghỉ (Giây): {vang}"))
    doiacc = int(input(f"{atuan}{luc}Sau Bao Nhiêu Nhiệm Vụ Thì Đổi Acc: {vang}"))
    print(f"{trang}═══════════════════════════════════════════════════════════")
    idx, nvblockx = 0, 0
    error_counts = {}
    seen_jobs = set()
    while True:
        ...
        if not list_clone:
            ...
            while True:
                stt += 1
                cookie_fb = input(f'{atuan}{luc}Nhập Cookie Facebook Thứ {xnhac}{stt}: {vang}')
                if cookie_fb == '': break
                fb = Facebook_API(cookie_fb)
                if not all([fb.idfb, fb.fb_dtsg, fb.jazoest]): print(f"{atuan}{do}Cookie không hợp lệ hoặc lỗi lấy thông tin."); stt -= 1
                else: print(f"{luc}{luc}Id Facebook: {vang}{fb.idfb} {do}</> {luc}Tên Tài Khoản: {vang}{fb.name}"); list_clone.append(cookie_fb); thanh_ngang()
